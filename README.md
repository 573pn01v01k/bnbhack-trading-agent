...
|---|---:|---:|---:|
| **DEX-liquid ensemble — full window (realistic cost)** | **−2.2%** | 0.04 | **18.7%** |
| DEX-liquid ensemble — locked 21d holdout | **+3.1%** | 3.65 | 2.5% |
| _(reference) same book @ optimistic 10 bps_ | +15.2% | 1.26 | 14.4% |
...
The live strategy is a **model-averaged ensemble**: regime-gated equal-weight over basket sizes N=3/4 of the DEX-liquid set (per-name cap 0.34) × regime MAs (240/336/480h), rebalanced every 4h, with two circuit-breakers — a **regime-hysteresis** gate (exit risk-off fast, re-enter only above MA×1.0075) and a **20% per-name trailing stop** (a held name 20% below its 24h peak drops to cash). Both are near-zero carry on a benign window and cap drawdown in the red-team's stress scenarios. Model-averaging is anti-overfit (never bets on one in-sample-best parameter); N=(3,4) is the validated sweet spot (N=(2,3) −4.6%, N=(4,5) −7.3%, holdout best at N=(3,4)).
...
- **Strategy** (`strategy.py`): the model-averaged ensemble above, over the **DEX-liquid set only** (`universe.dex_liquid_candidates` — ranked by measured on-chain depth, not CEX volume). Hold the basket when BTC is risk-on (above its regime MA, with hysteresis), rotate to the stablecoin leg otherwise; a per-name trailing stop drops any held name that craters. The same function drives backtest and live (decision parity).
